...
enum { MAX_RANK = 5, METADATA_SIZE = 48, MAX_ELEMENTS = 4096 };
...
static void set_tensor(int64_t metadata[METADATA_SIZE], int tensor,
  ...
  int64_t base = 3 + tensor * 15;
  for (int64_t dim = 0; dim < MAX_RANK; ++dim) {
    metadata[base + dim] = dim < rank ? extents[dim] : 1;
    metadata[base + 5 + dim] = dim < rank ? strides[dim] : 0;
    metadata[base + 10 + dim] = dim < rank ? modes[dim] : -1;
  }
}
...
static int64_t tensor_span(const int64_t metadata[METADATA_SIZE], int tensor) {
  ...
  for (int64_t dim = 0; dim < rank; ++dim)
    span += (metadata[base + dim] - 1) * metadata[base + 5 + dim];
  return span;
}
...
static void reference_contraction(
  ...
  int64_t mode_extents[MAX_RANK] = {1, 1, 1, 1, 1};
  int present[3][MAX_RANK] = {{0}};
  for (int tensor = 0; tensor < 3; ++tensor) {
    int64_t rank = metadata[tensor];
    int64_t base = 3 + tensor * 15;
    for (int64_t dim = 0; dim < rank; ++dim) {
      int64_t mode = metadata[base + 10 + dim];
  ...
  for (int64_t linear = 0; linear < total; ++linear) {
    ...
    for (int tensor = 0; tensor < 3; ++tensor) {
      ...
      for (int64_t dim = 0; dim < rank; ++dim)
        offsets[tensor] +=
            coordinates[metadata[base + 10 + dim]] *
            metadata[base + 5 + dim];
    }
