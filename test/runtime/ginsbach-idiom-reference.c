...
int main(void) {
  static double grid[64], rhs[64], residual[64] = {0};
  for (int i = 0; i < 64; ++i) grid[i] = 1.0, rhs[i] = 100.0;
  static const double a[4] = {2.0, 0.0, 3.0, 5.0};
  polygeist_mg_resid_f64(grid, rhs, residual, 4, 4, 4, a);
  if (!closed(residual[21], 22.0)) return 1;

  static double correction[64], unit_residual[64];
  for (int i = 0; i < 64; ++i)
    correction[i] = 2.0, unit_residual[i] = 1.0;
  static const double c[4] = {1.0, 2.0, 3.0, 0.0};
  polygeist_mg_psinv_f64(unit_residual, correction, 4, 4, 4, c);
  if (!closed(correction[21], 51.0)) return 2;

  static const int32_t values[7] = {1, 1, 1, 2, 2, -1, 9};
  static uint8_t bins[4] = {0, 254, 3, 0};
  polygeist_histogram_saturating_u8(values, bins, 7, 4);
  if (bins[0] != 0 || bins[1] != 255 || bins[2] != 5 || bins[3] != 0)
    return 3;

  static const float points[9] = {1, 0, 0, 0, 1, 0, -1, 0, 0};
  static const float bounds[3] = {0.75f, 0.0f, -1.0f};
  static int64_t pair_bins[4] = {0, 0, 0, 0};
  polygeist_tpacf_histogram_f32(points, 3, points, 3, 1,
                                pair_bins, 2, bounds);
  if (pair_bins[0] + pair_bins[1] + pair_bins[2] + pair_bins[3] != 3)
    return 4;

  static const int32_t nzcnt[2] = {2, 1}, jptr[2] = {0, 2};
  static const int32_t jcols[3] = {0, 1, 1}, perm[2] = {1, 0};
  static const float jdata[3] = {2, 3, 4}, jx[2] = {5, 7};
  static float jout[2] = {0, 0};
  polygeist_jds_spmv_f32(2, nzcnt, jptr, jcols, jdata, jx, perm, jout);
  if (!closef(jout[0], 21.0f) || !closef(jout[1], 38.0f)) return 5;

  static const int32_t rowptr[3] = {0, 2, 3}, cols[3] = {0, 1, 1};
  static const double data[3] = {2, 4, 3}, x[2] = {5, 7};
  static double out[2] = {0, 0};
  polygeist_csr_spmv_f64(2, rowptr, cols, data, x, out);
  if (!closed(out[0], 38.0) || !closed(out[1], 21.0)) return 6;

  static const float ga[6] = {1, 2, 3, 4, 5, 6};
  static const float gb[6] = {7, 8, 9, 10, 11, 12};
  static float gc[4] = {1, 1, 1, 1};
