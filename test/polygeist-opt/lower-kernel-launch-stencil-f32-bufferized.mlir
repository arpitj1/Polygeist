...
module {
  ...
  func.func @bufferized_stencil(
      %a0: memref<?x?x?xf32>, %a1: memref<?x?x?xf32>,
      %a2: memref<?x?x?xf32>, %a3: memref<?x?x?xf32>,
      %a4: memref<?x?x?xf32>, %a5: memref<?x?x?xf32>,
      %a6: memref<?x?x?xf32>, %out: memref<?x?x?xf32>, %c: f32) {
    kernel.launch @customStencil3D7pt_f32_tensor(
        %a0, %a1, %a2, %a3, %a4, %a5, %a6, %out,
        %c, %c, %c, %c, %c, %c, %c, %c, %c, %c)
        {polygeist.bufferized, polygeist.result_destinations = array<i64: 7>}
        : (memref<?x?x?xf32>, memref<?x?x?xf32>, memref<?x?x?xf32>,
           memref<?x?x?xf32>, memref<?x?x?xf32>, memref<?x?x?xf32>,
           memref<?x?x?xf32>, memref<?x?x?xf32>, f32, f32, f32, f32,
           f32, f32, f32, f32, f32, f32) -> ()
    return
  }
}

// CHECK-LABEL: func.func @bufferized_stencil
