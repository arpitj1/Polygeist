...
module {
  ...
  func.func @trisolv(%n: i32, %a: memref<?x?xf64>, %x: memref<?xf64>,
                     %b: memref<?xf64>) {
    ...
    affine.for %i = 0 to %ni {
      ...
      %arow = polygeist.submap(%a, %i, %last) {map = #row} : (memref<?x?xf64>, index, index) -> memref<?xf64>
      ...
      %diagonal = affine.load %a[%i, %i] : memref<?x?xf64>
  ...
  func.func @cholesky(%n: i32, %a: memref<?x?xf64>) {
    %one = arith.constant 1 : index
    %ni = arith.index_cast %n : i32 to index
    affine.for %i = 0 to %ni {
      affine.for %j = 0 to #id(%i) {
        %last = arith.subi %i, %one : index
        %left = polygeist.submap(%a, %i, %last) {map = #row} : (memref<?x?xf64>, index, index) -> memref<?xf64>
        %right = polygeist.submap(%a, %j, %last) {map = #row} : (memref<?x?xf64>, index, index) -> memref<?xf64>
        %out = polygeist.submap(%a, %i, %j, %last) {map = #coord} : (memref<?x?xf64>, index, index, index) -> memref<?xf64>
        linalg.generic {indexing_maps = [#id, #id, #id], iterator_types = ["reduction"]}
        ...
        %d = affine.load %a[%j, %j] : memref<?x?xf64>
        %v = affine.load %a[%i, %j] : memref<?x?xf64>
        %q = arith.divf %v, %d : f64
        affine.store %q, %a[%i, %j] : memref<?x?xf64>
      }
      %last = arith.subi %ni, %one : index
      %row_view = polygeist.submap(%a, %i, %last) {map = #row} : (memref<?x?xf64>, index, index) -> memref<?xf64>
      %diagonal_view = polygeist.submap(%a, %i, %last) {map = #diag} : (memref<?x?xf64>, index, index) -> memref<?xf64>
      ...
      affine.store %root, %a[%i, %i] : memref<?x?xf64>
    }
    return
  }
}
