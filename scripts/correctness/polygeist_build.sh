...
N_CALL=$(grep -cE 'call @polygeist_(cublas|cudnn|rmsnorm)' $WORK/abi.mlir || true)
...
$CC -O2 -c $WORK/wrapper.c -o $WORK/wrapper.o

# Original .c compiled normally; weaken the kernel symbol so the linker
# picks the lifted+matched version from kernel.o instead.
$CC -O2 "${GCC_PASSTHROUGH[@]}" -c "$INPUT" -o $WORK/harness_full.o
if [ "$TARGET" = "host" ]; then
...
POLYBENCH_OBJS=()
if grep -q '#include\s*<polybench.h>\|#include\s*"polybench.h"' "$INPUT"; then
