#!/bin/bash
# pva_boxfilter_jetson.sh — end-to-end test of the OpBoxFilter PVA path.
# Skips the matcher (which doesn't yet emit pvaBoxFilter_*) and hand-
...
#
# Usage: ./pva_boxfilter_jetson.sh <DTYPE> [SIZE]
#   <DTYPE>: i8 | i16
#   [SIZE]:  default 256
#
# Output: /tmp/pva_boxfilter_<DTYPE>_<SIZE>/{boxfilter_jetson, boxfilter_jetson_cpustub}
...
OUT=/tmp/pva_boxfilter_${DTYPE}_${SIZE}
mkdir -p $OUT
...
echo "[boxfilter/$DTYPE/$SIZE] (1) author kernel.launch MLIR by hand"
cat > $OUT/synth.mlir <<EOF
module {
  kernel.defn @pvaBoxFilter_3x3_${DTYPE}(
  ...
  func.func @kernel_conv2d(%ni: i32, %nj: i32,
    ...
    %Bc = memref.cast %Bv
       ...
       to memref<?x?x${MTY}, strided<[?, 1], offset: ?>>
    kernel.launch @pvaBoxFilter_3x3_${DTYPE}(%Ac, %Bc)
...
echo "[boxfilter/$DTYPE/$SIZE] (2) lower-kernel-launch-to-pva"
polygeist-opt --lower-kernel-launch-to-pva $OUT/synth.mlir -o $OUT/abi.mlir 2>$OUT/abi.err

echo "[boxfilter/$DTYPE/$SIZE] (3) lower to LLVM, translate, retarget aarch64"
...
echo "[boxfilter/$DTYPE/$SIZE] (4) cross-compile harness + wrapper + runtimes"
...
echo "[boxfilter/$DTYPE/$SIZE] (5) link PVA binary"
...
aarch64-linux-gnu-gcc -O2 \
    ...
    -o $OUT/boxfilter_jetson

echo "[boxfilter/$DTYPE/$SIZE] (6) link CPU-stub binary"
aarch64-linux-gnu-gcc -O2 \
    $OUT/main.o $OUT/wrapper.o $OUT/kernel.o $OUT/rt_cpu.o \
    -lm -lpthread -o $OUT/boxfilter_jetson_cpustub

echo ""
echo "═══ boxfilter ${DTYPE} ${SIZE}×${SIZE} binaries ═══"
ls -la $OUT/boxfilter_jetson $OUT/boxfilter_jetson_cpustub
