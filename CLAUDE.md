...
```bash
cd /home/arjaiswal/Polygeist/build && ninja
```
...
polygeist-opt <input_debufferized.mlir> --linalg-to-kernel="kernel-library-path=/home/arjaiswal/Polygeist/generic_solver/kernel_library.mlir"
```
...
The directory `/home/arjaiswal/pva-solutions/` is the source tree for the PVA
...
- *Build-time include path is fine.* Add `-I/home/arjaiswal/pva-solutions/public/src/operator/include`
...
- *Never copy headers into the Polygeist tree.* No `cp` / `git add` of any
  `.h` / `.hpp` / `.cpp` / `.c` from `/home/arjaiswal/pva-solutions/` into
  `/home/arjaiswal/Polygeist/`. The Polygeist repo only ever references those
  paths symbolically.
